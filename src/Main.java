...
public class Main {
	...
	public static void runApp() {
		...
		for(String s: flightHashMap.keySet()) {
			...
			data[i][6] = flightHashMap.get(s).getDeparture().getTower().getGpsCoordinate().getLatitude();
			data[i][7] = flightHashMap.get(s).getDeparture().getTower().getGpsCoordinate().getLongitude();
			data[i][8] = "On Flight";
			i++;
		}
			...
			public void valueChanged(ListSelectionEvent event) {
				if(!event.getValueIsAdjusting()) {
					...
					try {
						...
						//System.out.println(flightHashMap.get(code).calculateDistance());
						double distance = flightHashMap.get(code).calculateDistance();
						DecimalFormat df = new DecimalFormat("##.##");
						jTextArea3.setText(df.format(distance));
                        double time = flightHashMap.get(code).calculateTime();
						jTextArea2.setText(df.format(time));
						double fuel = flightHashMap.get(code).consumption();
						jTextArea1.setText(df.format(fuel));
						double co2 = flightHashMap.get(code).emission();
						jTextArea.setText(df.format(co2));
					}
					catch (Exception e) {		
					}
		...
		addBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				d[6] = flightHashMap.get(flight.getCode()).getDateTime().toString();
				d[7] = flightHashMap.get(flight.getCode()).getDateTime().toString();
				d[8] = flightHashMap.get(flight.getCode()).getDateTime().toString();
